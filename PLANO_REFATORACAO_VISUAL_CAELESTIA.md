...
- [ ] Remover código e convenções visuais legadas restantes (além da consolidação de cantos já feita).
...
**Último trabalho concluído:** Todo o trabalho já implementado e validado das Fases 3 (widgets restantes) a 8 estava correto mas nunca tinha sido commitado (175 arquivos, ~6.500 linhas, acumulados em uma única árvore de trabalho); reconciliado em oito commits separados por fase nesta sessão. Nesse processo, o `qs -p` (Lab) revelou-se quebrado desde a Fase 4: quatro arquivos usavam `NAnim`/`NColorAnimation` sem importar `qs.Widgets`, então nenhuma validação visual real era possível fazia tempo apesar do ledger registrar gates aprovados. Corrigido, e a duplicação de cálculo de canto entre `SmartPanel`/`LauncherOverlayWindow` (achado da Fase 8) foi consolidada no `ShapeCornerHelper`. Auditoria de `Easing.*` fora das primitivas concluída: zero ocorrências não documentadas. `qs -p` confirmado carregando limpo (zero erro QML, todos os serviços iniciados) via processo gerenciado com captura de log completo; monitor DP-2 não estava conectado nesta sessão, então a validação ficou limitada a DP-1.

**Próxima ação exata:** Fase 9 — com o Lab voltando a carregar, executar a matriz visual completa (light/dark, esquema fixo/wallpaper, blur on/off, fullscreen, escala, raios, animação desligada, performance mode) em DP-1 e, quando DP-2 estiver disponível, repetir em dois monitores; confirmar os 55 widgets/25 SmartPanels/superfícies independentes um a um; eliminar durações literais de animação restantes; auditar convenções visuais legadas além da já resolvida; só então mesclar em `legacy-v4` e sincronizar a shell ativa.

**Bloqueios conhecidos:** nenhum bloqueio de ferramenta; a validação em dois monitores depende de DP-2 estar fisicamente conectado na máquina de desenvolvimento.
...
| 2026-08-18 | 9 | `bash Scripts/dev/build-visual-plugin.sh` reconstruiu `Hydra.Visual`/`M3Shapes` com o `plugin/CMakeLists.txt` atualizado; `qs -p` executado como processo gerenciado (`hub start`) com `QML_IMPORT_PATH` apontando para o install local rodou 47s em DP-1 real sem nenhuma linha `ERROR`, com todos os serviços (`HyprlandService` até `Network`) reportando início | Regressão de carregamento do Lab, presente desde a Fase 4, corrigida e verificada; matriz visual completa (light/dark, wallpaper, blur, fullscreen, escala, DP-2) ainda não executada — próximo passo da Fase 9 |
