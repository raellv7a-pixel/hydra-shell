...
Singleton {
  ...
  property bool hydraPerformanceMode: false
  ...
  // Hydra Performance Mode
  // - Turning shadow off
  // - Turning animation off
  // - Do Not Disturb
  function toggleHydraPerformance() {
    hydraPerformanceMode = !hydraPerformanceMode;
  }

  function setHydraPerformance(value) {
    hydraPerformanceMode = value;
  }

  onHydraPerformanceModeChanged: {
    if (hydraPerformanceMode) {
      ToastService.showNotice(I18n.tr("toast.hydra-performance.label"), I18n.tr("toast.hydra-performance.enabled"), "rocket");
    } else {
      ToastService.showNotice(I18n.tr("toast.hydra-performance.label"), I18n.tr("toast.hydra-performance.disabled"), "rocket-off");
    }
  }
}
