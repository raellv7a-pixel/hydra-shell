...
Singleton {
  ...
  readonly property var _metadataSource: MediaService.currentPlayer && MediaService.currentPlayer._stateSource ? MediaService.currentPlayer._stateSource : MediaService.currentPlayer
  readonly property string _observedArtist: String(_metadataSource && _metadataSource.trackArtist !== undefined ? _metadataSource.trackArtist : (MediaService.trackArtist || "")).trim()
  readonly property string _observedTitle: String(_metadataSource && _metadataSource.trackTitle !== undefined ? _metadataSource.trackTitle : (MediaService.trackTitle || "")).replace(/(\r\n|\n|\r)/g, "").trim()
  readonly property string _observedAlbum: String(_metadataSource && _metadataSource.trackAlbum !== undefined ? _metadataSource.trackAlbum : (MediaService.trackAlbum || "")).trim()
  readonly property real _observedDuration: {
    const duration = Number(_metadataSource && _metadataSource.length !== undefined ? _metadataSource.length : (MediaService.trackLength || 0));
  ...
  function _search(artist, title, album, duration, normalized, trackKey, requestId) {
    ...
    let url = "https://lrclib.net/api/search?artist_name=" + encodeURIComponent(searchArtist) + "&track_name=" + encodeURIComponent(searchTitle);
  ...
  function _normalizedTitle(title) {
    return String(title || "").replace(/\s*[([]\s*(?:feat\.?|ft\.?)\s+[^\])]+[\])]/i, "").replace(/\s+-\s+(?:remaster(?:ed)?|live|radio edit).*$/i, "").trim();
  }
  ...
  function _parseSyncedLyrics(syncedText) {
    ...
    for (let i = 0; i < sourceLines.length; i++) {
      ...
      if (!match)
        continue;
      const text = sourceLine.replace(/\[\d+:\d+(?:\.\d+)?\]/g, "").replace(/<\d+:\d+(?:\.\d+)?>/g, "").trim();
    ...
    parsedLines.sort(function (left, right) {
