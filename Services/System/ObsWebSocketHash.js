...
function toBase64(bytes) {
  ...
  for (let index = 0; index < bytes.length; index += 3) {
    ...
    const chunk = (bytes[index] << 16) | ((secondPresent ? bytes[index + 1] : 0) << 8) | (thirdPresent ? bytes[index + 2] : 0)
    result += alphabet[(chunk >> 18) & 0x3f]
    result += alphabet[(chunk >> 12) & 0x3f]
    result += secondPresent ? alphabet[(chunk >> 6) & 0x3f] : "="
    result += thirdPresent ? alphabet[chunk & 0x3f] : "="
  }
  return result
}
...
function sha256Base64(value) {
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 0; index < 64; ++index) {
      ...
      const temp1 = add32(h, sum1, choose, CONSTANTS[index], words[index])
      const sum0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22)
      const temp2 = add32(sum0, (a & b) ^ (a & c) ^ (b & c))
  ...
  for (const value of state)
    digest.push((value >>> 24) & 0xff, (value >>> 16) & 0xff, (value >>> 8) & 0xff, value & 0xff)
