...
Item {
  id: root
  property var pluginApi: ScreenToolkitService.provider
  ...
  readonly property var ocrLangModel: {
    ...
    for (var i = 0; i < root.installedLangs.length; i++)
      out.push({
                 key: root.installedLangs[i],
                 name: root.installedLangs[i].toUpperCase()
               });
    return out;
  }
  onActiveToolChanged: {
  ...
  onRecordStateChanged: {
    if (recordState !== "") {
      viewedTool = "record";
      if (mainInstance)
        root.recordPath = mainInstance.recordPath;
  ...
  readonly property var toolDefs: pluginApi ? [
                                                {
                                                  icon: "color-picker",
                                                  label: pluginApi.tr("tools.colorpicker"),
                                                  tool: "colorpicker",
                                                  tooltip: pluginApi.tr("tooltips.colorpicker")
                                                },
                                                {
                                                  icon: "palette",
                                                  label: pluginApi.tr("tools.palette"),
                                                  tool: "palette",
                                                  tooltip: pluginApi.tr("tooltips.palette")
                                                },
                                                {
                                                  icon: "scan",
                                                  label: pluginApi.tr("tools.ocr"),
                                                  tool: "ocr",
                                                  tooltip: pluginApi.tr("tooltips.ocr")
                                                },
                                                {
                                                  icon: "world-search",
                                                  label: pluginApi.tr("tools.lens"),
                                                  tool: "lens",
                                                  tooltip: pluginApi.tr("tooltips.lens")
                                                },
                                                {
                                                  icon: "qrcode",
                                                  label: pluginApi.tr("tools.qr"),
                                                  tool: "qr",
                                                  tooltip: pluginApi.tr("tooltips.qr")
                                                },
                                                {
                                                  icon: "brush",
                                                  label: pluginApi.tr("tools.annotate"),
                                                  tool: "annotate",
                                                  tooltip: pluginApi.tr("tooltips.annotate")
                                                },
                                                {
                                                  icon: "video",
                                                  label: pluginApi.tr("tools.record"),
                                                  tool: "record",
                                                  tooltip: pluginApi.tr("tooltips.record")
                                                },
                                                {
                                                  icon: "pin",
                                                  label: pluginApi.tr("tools.pin"),
                                                  tool: "pin",
                                                  tooltip: pluginApi.tr("tooltips.pin")
                                                },
                                                {
                                                  icon: "ruler",
                                                  label: pluginApi.tr("tools.measure"),
                                                  tool: "measure",
                                                  tooltip: pluginApi.tr("tooltips.measure")
                                                },
                                                {
                                                  icon: "camera",
                                                  label: pluginApi.tr("tools.mirror"),
                                                  tool: "mirror",
                                                  tooltip: pluginApi.tr("tooltips.mirror")
                                                }
                                              ] : []
  ...
  function triggerFocused() {
    if (root.focusedTool < 0 || root.focusedTool >= root.toolDefs.length)
      return;
    var t = root.toolDefs[root.focusedTool].tool;
    if (root.isRunning) {
      Logger.w("ScreenToolkit", "blocked: isRunning");
      return;
    }
    root.viewedTool = t;
    if (t === "colorpicker" && root._savedHex !== "")
      return;
    if (t === "palette" && root._savedPalette.length > 0)
      return;
    if (t === "qr" && root._savedQr !== "")
      return;
    if (t === "mirror" && root.mirrorActive)
      return;
    if (t === "record") {
      if (root.isRecording)
        root.mainInstance?.runRecordStop();
      return;
    }
    if (t === "ocr" || t === "pin" || t === "annotate")
      return;
    if (t === "colorpicker")
      root.mainInstance?.runColorPicker();
    else if (t === "qr")
      root.mainInstance?.runQr();
    else if (t === "lens")
      root.mainInstance?.runLens();
    else if (t === "measure")
      root.mainInstance?.runMeasure();
    else if (t === "palette")
      root.mainInstance?.runPalette();
    else if (t === "mirror")
      root.mainInstance?.runMirror();
  }
    ...
    Column {
      ...
      Rectangle {
        ...
        Keys.onPressed: function (event) {
          if (event.key === Qt.Key_Left) {
            root.focusedTool = root.focusedTool < 0 ? 0 : (root.focusedTool + 9) % 10;
            event.accepted = true;
          } else if (event.key === Qt.Key_Right) {
            root.focusedTool = root.focusedTool < 0 ? 0 : (root.focusedTool + 1) % 10;
            event.accepted = true;
          } else if (event.key === Qt.Key_Up) {
            root.focusedTool = root.focusedTool >= 5 ? root.focusedTool - 5 : (root.focusedTool < 0 ? 0 : root.focusedTool);
            event.accepted = true;
          } else if (event.key === Qt.Key_Down) {
            root.focusedTool = root.focusedTool < 0 ? 0 : (root.focusedTool < 5 ? root.focusedTool + 5 : root.focusedTool);
            event.accepted = true;
          ...
          Row {
            ...
            Repeater {
              ...
              delegate: ToolBtn {
                ...
                hasResult: modelData.tool === "colorpicker" ? root._savedHex !== "" : modelData.tool === "palette" ? root._savedPalette.length > 0 : modelData.tool === "ocr" ? root._savedOcr !== "" : modelData.tool === "qr" ? root._savedQr !== "" : false
                width: toolsCol.btnSize
                height: toolsCol.btnSize + 18
                onTriggered: {
                  root.focusedTool = myIdx;
          ...
          Row {
            ...
            Repeater {
              ...
              delegate: ToolBtn {
                ...
                running: root.isRunning
                recording: root.isRecording && modelData.tool === "record"
                hasResult: modelData.tool === "record" ? root.isDone : modelData.tool === "mirror" ? root.mirrorActive : modelData.tool === "pin" ? (mainInstance?.hasPins ?? false) : false
                width: toolsCol.btnSize
                height: toolsCol.btnSize + 18
                onTriggered: {
                  root.focusedTool = myIdx;
      ...
      Rectangle {
        ...
        Row {
          ...
          NText {
            text: pluginApi?.tr("panel.running")
      ...
      Column {
        ...
        Rectangle {
          ...
          Image {
            ...
            source: root.recordFormat === "mp4" && root._thumbBust !== "" ? "file:///tmp/screen-toolkit-record-thumb.png?" + root._thumbBust : ""
            fillMode: Image.PreserveAspectFit
            smooth: true
            cache: false
          }
        ...
        Row {
          ...
          Rectangle {
            ...
            NIcon {
              anchors.centerIn: parent
              icon: "trash"
              color: recDiscardBtn.containsMouse ? Color.mError : Color.mOnSurfaceVariant
            }
            MouseArea {
              id: recDiscardBtn
              anchors.fill: parent
            ...
            model: [
              {
                id: "gif",
                label: "GIF",
                hint: "· " + (pluginApi?.pluginSettings?.gifMaxSeconds ?? 30) + "s"
              },
              {
                id: "mp4",
                label: "MP4",
  ...
  component ToolBtn: Item {
    ...
    readonly property color _accentColor: recording ? Color.mError : active ? Color.mPrimary : Color.mSecondary
    signal triggered
    Column {
      anchors.centerIn: parent
      spacing: Style.spaceXS
