...
Item {
  ...
  function run(grimX, grimY, grimW, grimH, langStr) {
    ...
    ocrProc.command = [root.scriptsDir + "ocr.sh", String(grimX), String(grimY), String(grimW), String(grimH), langStr || "eng", upscale, psm];
    ocrProc.running = true;
  }

  function runTranslate(text, targetLang) {
    if (!text || text === "" || root.isTranslating)
      return;
    root.isTranslating = true;
    root.translateResult = "";
    translateProc.exec({
                         command: ["bash", "-c", "trans -brief -to " + targetLang + " '" + text.replace(/'/g, "'\\''") + "'"]
                       });
  }
  ...
  Process {
    ...
    onExited: {
      ...
      root.translateResult = result !== "" ? result : root.pluginApi?.tr("messages.translate-failed");
    }
  }
}
