...
Item {
  ...
  readonly property var transLangModel: {
    ...
    for (var i = 0; i < transLangs.length; i++)
      out.push({
                 key: transLangs[i].code,
                 name: transLangs[i].name
               });
    return out;
  }
  ...
  function _translate(text, lang) {
    if (mainInstance)
      mainInstance.runTranslate(text, lang);
  }
  function clear() {
    if (mainInstance) {
    ...
    Rectangle {
      ...
      Flickable {
        ...
        TextEdit {
          ...
          WheelHandler {
            onWheel: event => {
                       ocrFlick.flick(0, event.angleDelta.y * 5);
      ...
      Row {
        ...
        Rectangle {
          ...
          NIcon {
            anchors.centerIn: parent
            icon: "world"
            color: transBtnMa.containsMouse ? Color.mOnPrimary : Color.mOnSurface
          }
          MouseArea {
            id: transBtnMa
            anchors.fill: parent
      ...
      Rectangle {
        ...
        Flickable {
          ...
          TextEdit {
            ...
            WheelHandler {
              onWheel: event => {
                         trFlick.flick(0, event.angleDelta.y * 5);
                         event.accepted = false;
                       }
            }
          }
