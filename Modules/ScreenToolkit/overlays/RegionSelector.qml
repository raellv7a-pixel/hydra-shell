...
Item {
  ...
  Variants {
    ...
    delegate: PanelWindow {
      ...
      WlrLayershell.namespace: "hydra-region-selector"
      ...
      Canvas {
        ...
        onPaint: {
          ...
          if (hasSel) {
            ...
            ctx.lineWidth = 1.5;
            ctx.strokeRect(sx, sy, sw, sh);
            var handles = [[sx, sy], [sx + sw / 2, sy], [sx + sw, sy], [sx + sw, sy + sh / 2], [sx + sw, sy + sh], [sx + sw / 2, sy + sh], [sx, sy + sh], [sx, sy + sh / 2]];
            var hs = 8;
      ...
      Rectangle {
        ...
        x: {
          ...
          return bx + width > win.width - 4 ? win.mouseX - width - 20 : bx;
        }
        y: {
          var by = win.mouseY + 20;
          return by + height > win.height - 4 ? win.mouseY - height - 20 : by;
        }
        NText {
          id: _coordText
          anchors.centerIn: parent
