...
Variants {
  ...
  function _annotateOutputDir() {
    ...
    if (custom.trim() !== "")
      return U.expandPath(custom.trim().replace(/\/$/, ""), Quickshell.env("HOME"));
    return "__auto__";
  }
  ...
  delegate: PanelWindow {
    ...
    mask: Region {
      item: !overlayWin.isPrimary ? null : fullMask
    }
    ...
    function _rebuildCache() {
      if (overlayWin._cacheRebuilding)
        return;
    ...
    function _drawStrokeToCtx(ctx, stroke) {
      ...
      if (stroke.type === "blur" && !stroke.preview) {
        ...
        ctx.lineTo(stroke.x2 - hs * Math.cos(angle - hw), stroke.y2 - hs * Math.sin(angle - hw));
        ctx.lineTo(stroke.x2 - hs * Math.cos(angle + hw), stroke.y2 - hs * Math.sin(angle + hw));
        ctx.closePath();
        ctx.fill();
        ...
        if (rx > 0 || ry > 0) {
          ctx.beginPath();
          ctx.ellipse(stroke.x1, stroke.y1, Math.max(rx, 1), Math.max(ry, 1), 0, 0, Math.PI * 2);
          ctx.stroke();
        }
    ...
    function requestZoom(scale) {
      ...
      if (scale === 1.0) {
        ...
        for (var ri = 0; ri < overlayWin.strokes.length; ri++)
          if (overlayWin.strokes[ri].type === "step")
            restoredCount++;
      ...
      zoomProc.exec({
                      command: ["bash", "-c", "magick /tmp/screen-toolkit-annotate.png -resize " + newW + "x" + newH + "! /tmp/screen-toolkit-annotate-zoom.png 2>/dev/null"]
                    });
    }
    Process {
      id: pixelateProc
    ...
    function preparePixelImage() {
      ...
      pixelImgReady = false;
      pixelateProc.exec({
                          command: ["bash", "-c", "magick /tmp/screen-toolkit-annotate.png -scale 5% -scale 2000% " + "/tmp/screen-toolkit-annotate-pixel.png 2>/dev/null"]
                        });
    }
    onPixelImgReadyChanged: {
      if (pixelImgReady) {
        var stale = "file:///tmp/screen-toolkit-annotate-pixel.png?" + (overlayWin._pixelCacheBust - 1);
    ...
    Process {
      ...
      onExited: code => {
                  if (code === 0) {
                    root.parseAndShowZoomed(root.lastRegion, "/tmp/screen-toolkit-annotate-zoom.png", overlayWin._pendingZoomScale);
                  }
                }
    }
    Process {
    ...
    Process {
      ...
      onExited: code => {
                  ...
                  if (code === 0) {
                    ...
                    ToastService.showNotice(root.mainInstance?.pluginApi?.tr("annotate.savedTo", {
                                                                               dest: dest
                                                                             }), dest, "device-floppy");
                    root.hide();
                  } else {
    ...
    Shortcut {
      sequence: "Ctrl+C"
      onActivated: {
        if (overlayWin.isSaving || !overlayWin.isPrimary)
          return;
                  ...
                  if (code === 0) {
                    ...
                    if (url.startsWith("http")) {
                      ...
                      if (skipPop) {
                        ...
                        wlCopyUrlProc.exec({
                                             command: ["bash", "-c", "printf '%s' " + U.shellEscape(url) + " | wl-copy"]
                                           });
                        ToastService.showNotice(root.mainInstance?.pluginApi?.tr("annotate.shareUrl"), url, "link");
                      }
                      return;
                    }
    ...
    Connections {
      ...
      function onIsVisibleChanged() {
        ...
        if (!root.isVisible) {
          ...
          drawCanvas.requestPaint();
          cleanupProc.exec({
                             command: ["bash", "-c", "rm -f /tmp/screen-toolkit-annotate-zoom.png"]
                           });
        } else {
    ...
    Rectangle {
      ...
      MouseArea {
        ...
        onClicked: mouse => {
                     ...
                     var inToolbar = mouse.x >= toolbar.x && mouse.x <= toolbar.x + toolbar.width && mouse.y >= toolbar.y && mouse.y <= toolbar.y + toolbar.height;
                     var inPopover = (overlayWin.showPopover && mouse.x >= popover.x && mouse.x <= popover.x + popover.width && mouse.y >= popover.y && mouse.y <= popover.y + popover.height) || (overlayWin.showSharePopover && mouse.x >= sharePopover.x && mouse.x <= sharePopover.x + sharePopover.width && mouse.y >= sharePopover.y && mouse.y <= sharePopover.y
                                                                                                                                                                                                   + sharePopover.height);
        ...
        Rectangle {
          ...
          MouseArea {
            ...
            onClicked: {
              overlayWin.showSharePopover = false;
              wlCopyUrlProc.exec({
                                   command: ["bash", "-c", "printf '%s' " + U.shellEscape(overlayWin.shareUrl) + " | wl-copy"]
                                 });
              ToastService.showNotice(root.mainInstance?.pluginApi?.tr("annotate.shareUrl"), overlayWin.shareUrl, "link");
            }
    ...
    Item {
      ...
      Image {
        ...
        x: root.zoomScale > 1.0 ? Math.max(root.regionW - width, Math.min(0, (root.regionW - width) / 2 + overlayWin.panX)) : 0
        y: root.zoomScale > 1.0 ? Math.max(root.regionH - height, Math.min(0, (root.regionH - height) / 2 + overlayWin.panY)) : 0
        source: root.isVisible && overlayWin.isPrimary ? "file://" + root.imagePath + "?v=" + root._resetToken : ""
    ...
    Rectangle {
      visible: overlayWin.isPrimary && root.zoomScale <= 1.0 && overlayWin.drawing && overlayWin.currentStroke !== null && overlayWin.currentStroke.type === "blur"
    ...
    Rectangle {
      ...
      Row {
        ...
        NText {
          text: root.mainInstance?.pluginApi?.tr("annotate.zoomViewOnly", {
    ...
    Canvas {
      ...
      function eraseAt(ex, ey) {
        ...
        for (var i = overlayWin.strokes.length - 1; i >= 0; i--) {
          ...
          if (s.points) {
            ...
            var lx = Math.min(s.x1, s.x2);
            var rx2 = Math.max(s.x1, s.x2);
            var ty = Math.min(s.y1, s.y2);
            var by2 = Math.max(s.y1, s.y2);
            hit = seg2(ex, ey, lx, ty, rx2, ty) < r2 || seg2(ex, ey, rx2, ty, rx2, by2) < r2 || seg2(ex, ey, rx2, by2, lx, by2) < r2 || seg2(ex, ey, lx, by2, lx, ty) < r2;
      ...
      MouseArea {
        ...
        onPositionChanged: mouse => {
                             ...
                             if (!overlayWin.drawing || !overlayWin.currentStroke)
                             return;
                             if (overlayWin.tool === "pencil" || overlayWin.tool === "highlighter") {
    ...
    Rectangle {
      ...
      property real _dragSy: 0
      property real _dragStx: 0
      property real _dragSty: 0
      readonly property var toolDefs: [
        {
          id: "pencil",
          icon: "pencil",
          tooltipKey: "annotate.toolPencil"
        },
        {
          id: "highlighter",
          icon: "highlight",
          tooltipKey: "annotate.toolHighlighter"
        },
        {
          id: "line",
          icon: "slash",
          tooltipKey: "annotate.toolLine"
        },
        {
          id: "arrow",
          icon: "arrow-up-right",
          tooltipKey: "annotate.toolArrow"
        },
        {
          id: "rect",
          icon: "square",
          tooltipKey: "annotate.toolRect"
        },
        {
          id: "circle",
          icon: "circle",
          tooltipKey: "annotate.toolCircle"
        },
        {
          id: "text",
          icon: "text-size",
          tooltipKey: "annotate.toolText"
        },
        {
          id: "blur",
          icon: "eye-off",
          tooltipKey: "annotate.toolBlur"
        },
        {
          id: "step",
          icon: "number-123",
          tooltipKey: "annotate.toolStep"
        },
        {
          id: "ruler",
          icon: "ruler-2",
          tooltipKey: "annotate.toolRuler"
        }
      ]
      ...
      readonly property var sizeDefs: [
        {
          size: 2,
          labelKey: "annotate.sizeS"
        },
        {
          size: 4,
          labelKey: "annotate.sizeM"
        },
        {
          size: 7,
          labelKey: "annotate.sizeL"
        }
      ]
      ...
      function doRedo() {
        if (overlayWin._redoStack.length > 0) {
          ...
          var count = 1;
          for (var i = 0; i < overlayWin.strokes.length; i++)
            if (overlayWin.strokes[i].type === "step")
              count++;
          overlayWin.stepCounter = count;
          overlayWin._invalidateCache();
          drawCanvas.requestPaint();
        ...
        Row {
          ...
          Repeater {
            ...
            ToolBtn {
              ...
              tip: root.mainInstance?.pluginApi?.tr(modelData.tooltipKey)
            }
          }
          ToolbarSeparator {}
          ZoomBtn {
          ...
          SaveBtn {
            iconName: "copy"
            labelText: overlayWin.isSaving ? root.mainInstance?.pluginApi?.tr("annotate.copying") : root.mainInstance?.pluginApi?.tr("annotate.copy")
          ...
          SaveBtn {
            ...
            labelText: overlayWin.isSaving ? root.mainInstance?.pluginApi?.tr("annotate.saving") : root.mainInstance?.pluginApi?.tr("annotate.save")
    ...
    function _doUpload(file) {
      ...
      uploadProc.exec({
                        command: ["bash", scriptPath, file, apiKey, expiry]
                      });
    }
    function flattenAndShare() {
      if (overlayWin.isUploading || overlayWin.isSaving)
        return;
      var skipPop = root.mainInstance?.pluginApi?.pluginSettings?.shareSkipPopover ?? false;
      ...
      if (root.zoomScale > 1.0) {
        ...
        drawCanvas.grabToImage(function (result) {
          ...
          flattenForShareProc.exec({
                                     command: ["bash", overlayWin._annotateScript, "share-flatten", "/tmp/screen-toolkit-annotate.png", "/tmp/screen-toolkit-overlay.png"]
                                   });
        });
      }
    }
    function flattenAndSave() {
      ...
      if (root.zoomScale > 1.0) {
        if (custom === "__auto__") {
          saveFileProc.exec({
                              command: ["bash", overlayWin._annotateScript, "save-auto", root.imagePath, filename, home + "/Pictures/Screenshots", home + "/Pictures"]
                            });
        } else {
          saveFileProc.exec({
                              command: ["bash", overlayWin._annotateScript, "save", root.imagePath, custom + "/" + filename]
                            });
        }
      } else {
        drawCanvas.grabToImage(function (result) {
          ...
          if (custom === "__auto__") {
            saveFileProc.exec({
                                command: ["bash", overlayWin._annotateScript, "save-overlay-auto", "/tmp/screen-toolkit-annotate.png", "/tmp/screen-toolkit-overlay.png", filename, home + "/Pictures/Screenshots", home + "/Pictures"]
                              });
          } else {
            saveFileProc.exec({
                                command: ["bash", overlayWin._annotateScript, "save-overlay", "/tmp/screen-toolkit-annotate.png", "/tmp/screen-toolkit-overlay.png", custom + "/" + filename]
                              });
          }
        });
      }
    ...
    function flattenAndCopy() {
      ...
      if (root.zoomScale > 1.0) {
        copyProc.exec({
                        command: ["bash", overlayWin._annotateScript, "copy-zoom", root.imagePath]
                      });
      } else {
        drawCanvas.grabToImage(function (result) {
          if (!result || !result.saveToFile("/tmp/screen-toolkit-overlay.png")) {
          ...
          clipFlattenProc.exec({
                                 command: ["bash", overlayWin._annotateScript, "copy", "/tmp/screen-toolkit-annotate.png", "/tmp/screen-toolkit-overlay.png"]
                               });
        });
      }
    }
  ...
  component ToolBtn: Rectangle {
    ...
    NIcon {
      anchors.centerIn: parent
      icon: iconName
      color: overlayWin.tool === toolId ? Color.mOnPrimary : tbHover.containsMouse ? Color.mOnHover : Color.mOnSurface
    }
    MouseArea {
      id: tbHover
      anchors.fill: parent
  ...
  component ActionBtn: Rectangle {
    ...
    NIcon {
      anchors.centerIn: parent
      icon: iconName
      color: (!parent.disabled && abHover.containsMouse) && parent.danger ? Color.mError : (!parent.disabled && abHover.containsMouse) ? Color.mOnHover : Color.mOnSurface
    }
    MouseArea {
      id: abHover
      anchors.fill: parent
