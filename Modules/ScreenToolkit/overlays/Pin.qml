...
Item {
  ...
  function _fileType(path) {
    ...
    if (ext === "gif")
      return "gif";
    if (["mp4", "webm", "mkv", "mov", "avi"].indexOf(ext) >= 0)
      return "video";
    return "image";
  }
  ...
  function updatePin(i, props) {
    if (i < 0 || i >= pinsModel.count)
      return;
    for (var k in props)
      pinsModel.setProperty(i, k, props[k]);
    root._maskVersion++;
  }
  Variants {
    ...
    delegate: PanelWindow {
      ...
      Rectangle {
        ...
        property var _bbox: {
          ...
          for (var i = 0; i < pinsModel.count; i++) {
            ...
            if (p.screenName !== pinWindow._screenName && p.screenName !== "")
              continue;
            found = true;
      ...
      Repeater {
        ...
        delegate: Item {
          ...
          visible: model.screenName === pinWindow._screenName || model.screenName === ""
          onVisibleChanged: {
            if (visible) {
              pinDelegate.x = model.posX;
              pinDelegate.y = model.posY;
          ...
          Rectangle {
            ...
            Image {
              ...
              source: pinDelegate.fileType === "image" && pinDelegate.pinImgPath !== "" ? "file://" + pinDelegate.pinImgPath : ""
            ...
            AnimatedImage {
              ...
              source: pinDelegate.fileType === "gif" && pinDelegate.pinImgPath !== "" ? "file://" + pinDelegate.pinImgPath : ""
              fillMode: pinDelegate.resolveFillMode(pinDelegate.fillMode)
            ...
            MediaPlayer {
              id: mediaPlayer
              source: pinDelegate.fileType === "video" && pinDelegate.pinImgPath !== "" ? "file://" + pinDelegate.pinImgPath : ""
              ...
              Component.onCompleted: {
                if (pinDelegate.fileType === "video" && pinDelegate.visible)
                  play();
              }
            ...
            Connections {
              ...
              function onVisibleChanged() {
                if (pinDelegate.fileType !== "video")
                  return;
                if (!pinDelegate.visible)
                  mediaPlayer.stop();
                else if (pinDelegate._playing)
                  mediaPlayer.play();
              }
            }
            VideoOutput {
            ...
            MouseArea {
              ...
              onPositionChanged: mouse => {
                                   if (!pinDelegate._dragging)
                                   return;
                                   var p = mapToItem(null, mouse.x, mouse.y);
                                   var par = pinDelegate.parent;
                                   pinDelegate.x = Math.max(-(pinDelegate.pinW - 60), Math.min(par ? par.width - 60 : 9999, _itemStartX + (p.x - _dragStart.x)));
                                   pinDelegate.y = Math.max(0, Math.min(par ? par.height - 40 : 9999, _itemStartY + (p.y - _dragStart.y)));
                                 }
              onReleased: mouse => {
                            if (mouse.button === Qt.RightButton)
                            return;
              ...
              Row {
                ...
                Item {
                  ...
                  MouseArea {
                    ...
                    onPositionChanged: mouse => {
                                         ...
                                         return;
                                         pinDelegate.pinOpacity = Math.max(0.05, Math.min(1.0, _startOp + (mouse.x - _startX) / opTrack.width));
                                       }
                    onReleased: root.updatePin(pinDelegate._myIndex, {
                                                 pinOpacity: pinDelegate.pinOpacity
                                               })
                  }
                }
                Rectangle {
                  width: Style.borderS
                  height: root._stripDivH
                  radius: Style.borderS
                  color: Qt.rgba(1, 1, 1, 0.25)
                  anchors.verticalCenter: parent.verticalCenter
                }
                Rectangle {
                  width: root._stripBtnSize
                  height: root._stripBtnSize
                ...
                Rectangle {
                  ...
                  MouseArea {
                    ...
                    preventStealing: true
                    onClicked: {
                      var next = pinDelegate.fillMode === "fit" ? "crop" : pinDelegate.fillMode === "crop" ? "stretch" : "fit";
            ...
            Rectangle {
              ...
              Column {
                ...
                MenuItem {
                  ...
                  visible: pinDelegate.fileType !== "video"
                  onActivated: root.updatePin(pinDelegate._myIndex, {
                                                fillMode: "fit"
                                              })
                }
                MenuItem {
  ...
  component ResizeCorner: MouseArea {
    ...
    onPressed: mouse => {
                 ...
                 _startPt = pt;
                 _startW = pinDelegate.pinW;
                 _startH = pinDelegate.pinH;
                 _startX = pinDelegate.x;
                 _startY = pinDelegate.y;
                 mouse.accepted = true;
               }
    onPositionChanged: mouse => {
                         if (!pressed)
                         ...
                         if (mode === 0) {
                           nw = Math.max(80, _startW + dx);
                           nh = Math.max(60, _startH + dy);
                         } else if (mode === 1) {
                           nw = Math.max(80, _startW - dx);
                           nh = Math.max(60, _startH + dy);
                           nx = _startX + (_startW - nw);
                         } else if (mode === 2) {
                           nw = Math.max(80, _startW + dx);
                           nh = Math.max(60, _startH - dy);
                           ny = _startY + (_startH - nh);
                         } else if (mode === 3) {
                           nw = Math.max(80, _startW - dx);
                           nh = Math.max(60, _startH - dy);
                           nx = _startX + (_startW - nw);
