...
Variants {
  ...
  delegate: PanelWindow {
    ...
    WlrLayershell.namespace: "hydra-measure"
    Shortcut {
      sequence: "Escape"
    ...
    readonly property var palette: ["#A78BFA", "#34D399", "#F87171", "#FBBF24", "#60A5FA", "#F472B6", "#A3E635", "#FB923C"]
    ...
    Timer {
      ...
      onTriggered: {
        ...
        shotProc.exec({
                        command: ["bash", script, String(sx), String(sy), String(rx), String(ry), String(rw), String(rh), String(lx1), String(ly1), String(lx2), String(ly2), String(lw), String(lh), overlayWin._shotColor, String(scale), destDir, fullPath]
                      });
      }
    }
    ...
    Rectangle {
      ...
      MouseArea {
        ...
        onPressed: mouse => {
                     ...
                     overlayWin.current = null;
                     overlayWin.x1 = mouse.x;
                     overlayWin.y1 = mouse.y;
                     overlayWin.x2 = mouse.x;
                     overlayWin.y2 = mouse.y;
                   }
        onReleased: mouse => {
                      ...
                      overlayWin.measuring = false;
                      var dist = Math.sqrt(Math.pow(overlayWin.x2 - overlayWin.x1, 2) + Math.pow(overlayWin.y2 - overlayWin.y1, 2));
                      if (dist > 4)
                      overlayWin.current = {
                        x1: overlayWin.x1,
                        y1: overlayWin.y1,
    ...
    Canvas {
      ...
      function drawLine(ctx, m, color) {
        ...
        if (w > 20) {
          ...
          ctx.setLineDash([]);
          ctx.beginPath();
          ctx.moveTo(Math.min(x1, x2), ty);
          ctx.lineTo(Math.max(x1, x2), ty);
          ctx.stroke();
          ctx.restore();
          ctx.fillStyle = "white";
        ...
        if (h > 20) {
          ...
          ctx.setLineDash([]);
          ctx.beginPath();
          ctx.moveTo(tx, Math.min(y1, y2));
          ctx.lineTo(tx, Math.max(y1, y2));
          ctx.stroke();
          ctx.restore();
          ctx.fillStyle = "white";
          ctx.font = "bold 11px sans-serif";
    ...
    Rectangle {
      ...
      x: {
        var rx = rawX;
        if (rx + width + 8 > overlayWin.width)
          rx = ex - width - 16;
        return Math.max(8, rx);
      }
      y: {
        var ry = rawY;
        if (ry + height + 8 > overlayWin.height)
          ry = ey - height - 16;
      ...
      Row {
        ...
        Rectangle {
          ...
          NIcon {
            anchors.centerIn: parent
            icon: "copy"
            color: acopyBtn.containsMouse ? Color.mOnPrimary : Color.mOnSurface
            scale: 0.85
          }
          MouseArea {
            id: acopyBtn
            ...
            onClicked: {
              measureVariants.copyResult(overlayWin.curDist + "px (" + Math.round(overlayWin.curW) + "×" + Math.round(overlayWin.curH) + ")");
              ToastService.showNotice(_tr("messages.measure-copied"));
            }
        ...
        Rectangle {
          ...
          NIcon {
            anchors.centerIn: parent
            icon: "camera"
            color: ascreenshotBtn.containsMouse ? Color.mOnPrimary : Color.mOnSurface
            scale: 0.85
          }
          MouseArea {
            id: ascreenshotBtn
        ...
        Rectangle {
          ...
          NIcon {
            anchors.centerIn: parent
            icon: "pin"
            color: pinBtn.containsMouse ? Color.mOnPrimary : Color.mOnSurface
            scale: 0.85
          }
          MouseArea {
            id: pinBtn
            ...
            onClicked: {
              overlayWin.doPin();
              ToastService.showNotice(_tr("messages.measure-pinned"));
            }
        ...
        Rectangle {
          ...
          NIcon {
            anchors.centerIn: parent
            icon: "x"
            color: discardBtn.containsMouse ? Color.mError : Color.mOnSurface
            scale: 0.85
          }
          MouseArea {
            id: discardBtn
            ...
            onClicked: {
              overlayWin.current = null;
              if (overlayWin.pinned.length === 0)
                measureVariants.hide();
            }
        ...
        Row {
          ...
          Rectangle {
            ...
            NIcon {
              anchors.centerIn: parent
              icon: "copy"
              color: pcopyBtn.containsMouse ? Color.mOnPrimary : Color.mOnSurface
              scale: 0.8
            }
            MouseArea {
              id: pcopyBtn
              ...
              cursorShape: Qt.PointingHandCursor
              onClicked: {
                measureVariants.copyResult(mdist + "px (" + Math.round(mw) + "×" + Math.round(mh) + ")");
                ToastService.showNotice(_tr("messages.measure-copied"));
              }
          ...
          Rectangle {
            ...
            NIcon {
              anchors.centerIn: parent
              icon: "camera"
              color: pscreenshotBtn.containsMouse ? Color.mOnPrimary : Color.mOnSurface
              scale: 0.8
            }
            MouseArea {
              id: pscreenshotBtn
          ...
          Rectangle {
            ...
            NIcon {
              anchors.centerIn: parent
              icon: "x"
              color: premoveBtn.containsMouse ? Color.mError : Color.mOnSurface
              scale: 0.8
            }
            MouseArea {
              id: premoveBtn
  ...
  function copyResult(txt) {
    if (mainInstance)
      mainInstance.copyToClipboard(txt);
  }
}
