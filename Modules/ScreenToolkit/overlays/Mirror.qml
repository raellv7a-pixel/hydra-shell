...
Item {
  ...
  function _startCountdown(action) {
    if (_countdownActive || _isRecording)
      return;
  ...
  function _fireAction() {
    _countdownActive = false;
    _countdown = 0;
    if (_pendingAction === "screenshot")
      _doScreenshot();
    else if (_pendingAction === "record")
      _doStartRecord();
    _pendingAction = "";
  }
  Timer {
  ...
  function _onImageCaptured(tmpPath) {
    ...
    screenshotProc.exec({
                          command: ["bash", scriptsDir + "mirror-screenshot.sh", tmpPath, dir, file, filters.join(",")]
                        });
  }
  Process {
    id: screenshotProc
    ...
    onExited: code => {
                if (code === 0) {
                  ...
                  ToastService.showNotice(root.pluginApi?.tr("mirror.screenshotSaved"), destPath, "camera");
                } else {
  ...
  function _doSaveRecord() {
    ...
    recSaveProc.exec({
                       command: ["bash", scriptsDir + "mirror-record.sh", root._recTmpPath, dir, file, filters.join(","), root._audioEnabled ? "1" : "0"]
                     });
  }
  Process {
    id: recSaveProc
    ...
    onExited: code => {
                ...
                root._recTmpPath = "";
                if (code === 0)
                ToastService.showNotice(root.pluginApi?.tr("mirror.recordSaved"), recSaveProc.savedPath, "device-floppy");
                else
  ...
  Variants {
    ...
    delegate: PanelWindow {
      ...
      readonly property bool isInteracting: isPrimary && (dragArea.pressed || resizeBR.pressed || resizeBL.pressed || resizeTR.pressed || resizeTL.pressed)
      Item {
        id: fullMask
        anchors.fill: parent
      }
      mask: Region {
        item: !win.isPrimary ? null : (win.isInteracting ? fullMask : container)
      }
      MediaDevices {
        id: mediaDevices
      }
      Rectangle {
          ...
          sourceComponent: Component {
            Item {
              ...
              Component.onDestruction: {
                if (root._imgCapture === imgCap)
                  root._imgCapture = null;
                if (root._recorder === mediaRec)
                  root._recorder = null;
              }
              CaptureSession {
                id: session
                ...
                imageCapture: ImageCapture {
                  ...
                  onErrorOccurred: (id, err, msg) => ToastService.showError(root.pluginApi?.tr("mirror.screenshotFailed"))
                }
        ...
        Rectangle {
          ...
          Row {
            ...
            NText {
              text: root.pluginApi?.tr("mirror.recordingInProgress", {
                                         time: root._formatTime(root._recElapsed)
                                       })
              color: "white"
            ...
            Rectangle {
              ...
              MouseArea {
                ...
                onClicked: {
                  ...
                  root.currentHeight = root.isSquare ? root.currentWidth : Math.max(100, Math.round(root.currentWidth * 9 / 16));
              ...
              MouseArea {
                ...
                onClicked: {
                  if (root._isRecording)
                    ...
                    root._startCountdown("screenshot");
                }
