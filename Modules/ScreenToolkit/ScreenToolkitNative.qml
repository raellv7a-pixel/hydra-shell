...
Item {
  ...
  Component.onCompleted: {
    ...
    root.activeTool = "";
    ScreenToolkitService.mainInstance = root;
  ...
  function _checkSession() {
    sessionCheckProc.exec({
                            command: ["bash", "-c", "[ -f /tmp/screen-toolkit-session ] && echo 'exists' || echo 'new'; " + "touch /tmp/screen-toolkit-session"]
                          });
  }
    ...
    function onRecordStateChanged() {
      ...
      if (recordOverlay.recordState === "converting" && !skipConfirm && !toClipboard) {
        ...
        if (screen)
          ...
          pluginApi.withCurrentScreen(sc => pluginApi.openPanel(sc));
      }
    }
    function onDismissed() {
      if (root.activeTool === "record")
        root.activeTool = "";
    }
  }
  Connections {
  ...
  Process {
    ...
    onExited: {
      ...
      if (pluginApi) {
        pluginApi.pluginSettings.detectedRecorder = path.endsWith("wl-screenrec") ? "wl-screenrec" : path.endsWith("wf-recorder") ? "wf-recorder" : "";
        pluginApi.saveSettings();
      }
      root.detectedRecorder = path.endsWith("wl-screenrec") ? "wl-screenrec" : path.endsWith("wf-recorder") ? "wf-recorder" : "";
    }
  }
  Process {
    id: annotateProc
  ...
  Timer {
    ...
    onTriggered: {
      ...
      annotateProc.exec({
                          command: ["bash", "-c", "grim -g \"" + root._grimGeometry + "\" /tmp/screen-toolkit-annotate.png 2>/dev/null"]
                        });
    }
  }
  Timer {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      annotateProc.exec({
                          command: name !== "" ? ["grim", "-o", name, "/tmp/screen-toolkit-annotate.png"] : ["grim", "/tmp/screen-toolkit-annotate.png"]
                        });
    }
  }
  Timer {
    ...
    onTriggered: {
      pinGrimProc.exec({
                         command: [root._scriptsDir + "capture.sh", "pin", root._grimGeometry]
                       });
    }
  }
  Timer {
    ...
    onTriggered: {
      pinFileProc.exec({
                         command: [root._scriptsDir + "pick-file.sh"]
                       });
    }
  }
  Timer {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      recordOverlay.startRecording(root._grimGeometry, root.pendingRecordFormat, root.pendingRecordAudioOut, root.pendingRecordAudioIn, root.pendingRecordCursor, root._grimLocalX, root._grimLocalY, root._regionScreen);
    }
  }
  Timer {
    ...
    onTriggered: {
      ...
      if (!screen)
        return;
      var scale = screen.devicePixelRatio ?? 1.0;
      var region = screen.x + "," + screen.y + " " + Math.round(screen.width * scale) + "x" + Math.round(screen.height * scale);
      root.isRunning = false;
      root.activeTool = "record";
      recordOverlay.startRecording(region, root.pendingRecordFormat, root.pendingRecordAudioOut, root.pendingRecordAudioIn, root.pendingRecordCursor, 0, 0, screen);
    }
  }
  Timer {
  ...
  function runRecordStop() {
    recordOverlay.stopRecording();
  }
  function runRecordSave() {
    recordOverlay._saveToFile();
  }
  function runRecordDiscard() {
    recordOverlay.dismiss();
  }
  function runRecord(format, audioOut, audioIn, cursor) {
    if (root.isRunning || recordOverlay.isRecording || recordOverlay.isConverting)
      return;
  ...
  function detectCapabilities() {
    ...
    detectRecorderProc.exec({
                              command: ["bash", "-c", "which wl-screenrec 2>/dev/null || which wf-recorder 2>/dev/null"]
                            });
  }
  function annotateScreenshotCmd(overlayTmpFile) {
    ...
    var dest = dir + "/" + fname;
    return "mkdir -p " + U.shellEscape(dir) + " && " + "magick /tmp/screen-toolkit-annotate.png " + U.shellEscape(overlayTmpFile) + " -composite " + U.shellEscape(dest) + " && " + "rm -f " + U.shellEscape(overlayTmpFile) + " && " + "echo " + U.shellEscape(dest);
  }
  function annotateScreenshotZoomCmd(imgPath) {
    var dir = U.screenshotDir(root._home, pluginApi?.pluginSettings?.screenshotPath);
    var fname = U.buildFilename("annotate", ".png", pluginApi?.pluginSettings?.filenameFormat);
    var dest = dir + "/" + fname;
    return "mkdir -p " + U.shellEscape(dir) + " && " + "cp " + U.shellEscape(imgPath) + " " + U.shellEscape(dest) + " && " + "echo " + U.shellEscape(dest);
  }
  IpcHandler {
    ...
    function ocr() {
      root.runOcr(pluginApi?.pluginSettings?.selectedOcrLang || "eng");
    }
    function qr() {
      root.runQr();
    }
    ...
    function recordFullscreen() {
      root.runRecordFullscreen("gif");
    }
    function recordFullscreenMp4() {
      root.runRecordFullscreen("mp4");
    }
    function recordStop() {
      if (recordOverlay.isRecording)
        recordOverlay.stopRecording();
    }
  }
}
